...
public class TestData {
  ...
  public void getResourcesInCities(String state, String city) {
	  ...
	  if(dataMap.keySet().contains(state) && dataMap.get(state).keySet().contains(city)) {
		  ...
		  for(WebElement resource : resourcesList) {
			  ...
			  resourcesMap.put(resourceName, null);
		  }
  ...
  public void verifyResources(String state, String city, String resource) {
	  ...
	  if(dataMap.get(state)!=null && dataMap.get(state).get(city)!=null) {
		  ...
		  Assert.assertTrue(dataMap.get(state).get(city).keySet().contains(resource));
	  } else {
		  Assert.assertTrue(false);
	  }
  }
  ...
  */
  ...
  public void getResourcesData(String state, String city, String resource) {
	  System.out.println("Entered getResourcesData function");
	  List<String> resourcesDataList = new ArrayList();
	  if(dataMap.keySet().contains(state) && dataMap.get(state).keySet().contains(city) && dataMap.get(state).get(city).keySet().contains(resource)) {
		  loadUrl(Url+state.split(" ")[0].toLowerCase()+"/"+city.split(" ")[0].toLowerCase()+"/"+resource.split(" ")[0].toLowerCase());
		  List<WebElement> resourcesList = driver.findElements(By.xpath("//*[@class='MuiTypography-root card-desc MuiTypography-body2']"));
		  for(WebElement resourceElement : resourcesList) {
			  String resourceName = resourceElement.getText().toLowerCase();
			  resourcesDataList.add(resourceName);
		  }
		  dataMap.get(state).get(city).put(resource, resourcesDataList);
		  System.out.println("Resources Data for state: "+state+" ---city: "+ city + "---resource : " +resource +" updated -- "+dataMap);
	  }
  }
  ...
  public void verifyResourcesData(String state, String city, String resource, String resourceData) {
	  ...
	  if((dataMap.get(state)!=null && !dataMap.get(state).isEmpty()) && (dataMap.get(state).get(city)!=null && !dataMap.get(state).get(city).isEmpty()) && dataMap.get(state).get(city).get(resource)==null) {
		  getResourcesData(state, city, resource);
	  }
	  //System.out.println("before assert------"+dataMap.get(state));
	  if(dataMap.get(state)!=null && dataMap.get(state).get(city)!=null && dataMap.get(state).get(city).get(resource)!=null) {
		  //System.out.println("in assert-----"+dataMap.get(state).get(city));
		  List<String> dataList = dataMap.get(state).get(city).get(resource);
		  boolean check = false;
		  for(String data : dataList) {
			  //System.out.println("-----------"+data+"---------------"+resourceData);
			  if(data.equalsIgnoreCase(resourceData)) {
				  check = true;
			  	  break;
			  } 
		  }
  ...
  public Object[] passExcelResourcesData() {
	  Object[][] stateCitiesResourcesList = passExcelData(4);
	  return stateCitiesResourcesList;
  }
